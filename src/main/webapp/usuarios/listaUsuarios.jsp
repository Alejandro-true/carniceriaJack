...
if (listaUsuarios != null) {
    for (Usuario u : listaUsuarios) {
        ...
        if ("ACTIVO".equals(u.getEstado())) activeCount++;
    }
}
%>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>🔐 Gestión de Usuarios — La Carnicería</title>
...
/* Username */
.user-name { font-weight: 600; font-size: .93rem; }
...
.role-badge.admin { background: #FFF8E6; color: #8B5E3C; }
...
<section class="hero-banner">
    <div class="container">
        <div class="hero-content">
            <div class="hero-left">
                <h1><i class="fas fa-user-shield" style="font-size:.75em; opacity:.6;"></i> Gestión de <span class="gold">Usuarios</span></h1>
                <p>Administra los usuarios del sistema, roles y permisos de acceso.</p>
            </div>
            <div class="hero-right mt-3 mt-md-0">
                <button class="btn-nuevo" onclick="modalUsuario.abrir('nuevo')">
...
<div class="stats-bar">
    ...
    <div class="stat-card">
        <div class="stat-icon blue"><i class="fas fa-shield-halved"></i></div>
        <div class="stat-info">
            <div class="stat-number"><%=(totalUsuarios - adminCount)%></div>
                ...
                <table class="table-carnic">
                    <thead>
                        <tr>
                            ...
                            <th><i class="fas fa-shield-halved"></i> Rol</th>
                    ...
                    if (listaUsuarios != null && !listaUsuarios.isEmpty()) {
                        for (Usuario usuario : listaUsuarios) {
                            ...
                            boolean isActive = "ACTIVO".equals(usuario.getEstado());
                    %>
                        <tr>
                            <!-- ID -->
                            <td><span class="id-badge"><%=usuario.getIdUsuario()%></span></td>

                            <!-- Usuario -->
                            <td><span class="user-name"><%=usuario.getNombreUsuario()%></span></td>
                            ...
                            <td>
                                <div class="actions-cell">
                                    ...
                                    <button class="btn-action password"
                                        onclick="modalUsuario.abrir('cambiarPassword', <%=usuario.getIdUsuario()%>)"
                                        title="Cambiar contraseña">
                                        <i class="fas fa-key"></i>
                                    </button>
                                    <button class="btn-action del"
                                        onclick="confirmarEliminar(<%=usuario.getIdUsuario()%>, '<%=usuario.getNombreUsuario()%>')"
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p><strong>No hay usuarios registrados.</strong><br>Agrega el primer usuario usando el botón <em>"Nuevo Usuario"</em>.</p>
                                </div>
                            </td>
                        </tr>
                    <%
...
<!-- ============================================================
     MODAL USUARIO (nuevo / editar / cambiar password)
     ============================================================ -->
<div class="modal fade modal-carnic" id="modalUsuario" tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>¿Estás seguro de eliminar al usuario <strong id="userNameConfirm"></strong>?<br><small class="text-muted">Esta acción no puede ser deshecha.</small></p>
            </div>
...
const modalUsuario = {
    ...
    init() {
        this.instance = new bootstrap.Modal(document.getElementById('modalUsuario'));
    },

    abrir(tipo, id = null) {
        this.resetear();
        let titulo = 'Usuario';
        let icono = 'fa-user-edit';
        
        if (tipo === 'nuevo') { titulo = 'Nuevo Usuario'; icono = 'fa-user-plus'; }
        else if (tipo === 'editar') { titulo = 'Editar Usuario'; icono = 'fa-pen'; }
        else if (tipo === 'cambiarPassword') { titulo = 'Cambiar Contraseña'; icono = 'fa-key'; }

        document.getElementById('modalUsuarioLabel').innerHTML = `<i class="fas ${icono}"></i> ${titulo}`;
    ...
    enviarFormulario(e, form) {
        ...
        if (!op || !op.endsWith('Ajax')) formData.set('op', op + 'Ajax');

        fetch('<%=url%>UsuariosController', {
            method: 'POST',
    ...
    deshabilitarBotones() {
        const btn = document.querySelector('#contenidoModal input[type="submit"], #contenidoModal button[type="submit"]');
        const close = document.getElementById('btnCerrarModal');
        if (btn) {
            btn.disabled = true;
            if (btn.tagName === 'BUTTON') btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Guardando…';
    ...
    habilitarBotones() {
        ...
        if (btn) {
            btn.disabled = false;
            if (btn.tagName === 'BUTTON') btn.innerHTML = '<i class="fas fa-save"></i> Guardar';
            else btn.value = 'Guardar';
        }
...
function confirmarEliminar(id, nombre) {
    _deleteUserId = id;
    document.getElementById('userNameConfirm').textContent = nombre;
...
document.getElementById('btnConfirmDel').addEventListener('click', function () {
    ...
    window.location.href = '<%=url%>UsuariosController?op=eliminar&id=' + _deleteUserId;
});

// ============================================================
// INIT
// ============================================================
document.addEventListener('DOMContentLoaded', () => modalUsuario.init());
</script>
